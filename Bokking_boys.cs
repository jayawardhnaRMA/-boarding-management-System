...
{
    public partial class Bokking_boys : Form
    {
        Function2 con;
        public Bokking_boys()
        {
        ...
        private void Bokking_boys_Load(object sender, EventArgs e)
        {
            try
            {
                string query = "SELECT * FROM boysRoomPrice";
                DataTable data = con.GetData(query);
                pricelist2.DataSource = data;
            }
            catch (Exception ex)
            {
